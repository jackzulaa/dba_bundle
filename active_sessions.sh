...
#		16-10-18 Added Checking Running Backups.
#		16-10-18 Fixed a bug of TOTALWORK=0 in v$session_longops
#		15-07-19 Display the command for PAUSE/RESUME/KILL a runnning RMAN backup.
#
#
#
#
...
select 'TOTAL:      '||count(*)         from gv\$session;
...
${SHOW_OS_COMMAND} select '*TO PAUSE THE RUNNING RMAN BACKUP RUN OS COMMAND:  kill -STOP '||listagg (p.spid, ' ')  WITHIN GROUP (ORDER BY p.spid) from v\$session s, v\$process p where s.program like 'rman@%' and p.addr=s.paddr; 
${SHOW_OS_COMMAND} select '*TO RESUME A "PAUSED" RMAN BACKUP RUN OS COMMAND:  kill -CONT '||listagg (p.spid, ' ')  WITHIN GROUP (ORDER BY p.spid) from v\$session s, v\$process p where s.program like 'rman@%' and p.addr=s.paddr; 
${SHOW_OS_COMMAND} select '*TO KILL  THE RUNNING RMAN BACKUP RUN OS COMMAND:  kill -9    '||listagg (p.spid, ' ')  WITHIN GROUP (ORDER BY p.spid) from v\$session s, v\$process p where s.program like 'rman@%' and p.addr=s.paddr; 
