...
#		05-09-18 Modified the wait in seconds column for 11gr2 compatibility.
#               20-10-18 Fixed divide by zero bug when TOTALWORK=0 in v$session_longops
#		26-03-19 Listing the Master Blocking Session details.
#               27-03-19 Used RULE Based Optimizer against all "checking locks" queries for a better performance on RAC instances.
#		01-08-19 Add more information to object level locks result.
#
#
#
# ####################################################################################################################################
...
prompt List of MASTER BLOCKING Sessions:
prompt -------------------------------------------------
...
prompt List of Victim LOCKED Sessions:
prompt -------------------------------------------------
...
prompt Lock Chain Analysis:
prompt -------------------------------------------------

set linesize 180
col BLOCKING_STATUS for a180
select /*+RULE*/ 'User: '||s1.username || ' | ' || s1.module || '(SID=' || s1.sid ||' ) running SQL_ID:'||s1.sql_id||'  is blocking
User: '|| s2.username || ' | ' || s2.module || '(SID=' || s2.sid || ') running SQL_ID:'||s2.sql_id||' For '||${WAIT_DISPLAY}||' Minutes
------------------------------------------------------------------------------
...
select  /*+RULE*/ OWNER||'.'||OBJECT_NAME "LOCKED_OBJECT", ORACLE_USERNAME||' | '||lo.OS_USER_NAME "LOCK HOLDER: DB_USER | OS_USER",l.sid||' | '|| lo.PROCESS "DB_SID | OS_PID",
        decode(TYPE,
                'MR', 'Media Recovery',
                'RT', 'Redo Thread',
                'UN', 'User Name',
