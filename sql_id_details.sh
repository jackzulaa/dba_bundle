...
col "PLSQL|JAVA TIME/EXEC_SEC" for a24
col "APP|USR_IO|CLS WAIT/EXEC_SEC" for a30
col "BUFF_GET|DISK_R|DIRECT_W/EXC" for a29
col P_MEM_MB for 99999999
col "ROWS/EXEC" for 999999999
col MODULE for a12
...
round(PERSISTENT_MEM/1024/1024,2) "P_MEM_MB",substr(MODULE,1,12)"MODULE", FIRST_LOAD_TIME, LAST_LOAD_TIME
...
echo "============================================================"
echo "Note: Please make sure that Diagnostic & Tuning License is already acquired before selecting YES."
while read ANS1
	do
        case ${ANS1} in
        ""|n|N|no|NO|No) break ;;
