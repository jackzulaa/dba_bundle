...
# To be run against hanged instance.
...
# If there is more than one DB ASK the user to select:
 ...
 then
    echo
    echo "Select ORACLE_SID: [Enter the number]"
    echo "-----------------"
...
while read ANALYZE_LEVEL
 do
                 ...
                 break ;;
                 *) echo
		    echo "Invalid Hang Analysis Level, please enter a number from this list [1,2,3,4,5,10]:"
		    echo "---------------------------------------------------------------------------------";;
...
echo
echo "Select the mode you want to run ORABEDUG with:"
echo "=============================================="
echo "[1] to run ORADEBUG against the DATABASE. [Default]"
echo "[2] to run ORADEBUG with SYSTEMSTATE option. [Default]"
while read ANS
 do
                 case $ANS in
                 "2"|"SYSTEM"|"system") echo;echo "ORADEBUG will analyze SYSTEM hang now ...";sleep 1
...
EOF
                 ...
                 break;;
                 ""|"1"|"DATABASE"|"database") echo; echo "ORADEBUG will analyze DATABASE hang now ..."
		 echo "[If this session hanged for more than a minute, kill it and run the script again with SYSTEMSTATE option.]";echo;sleep 1;

# I know it's not wise to connect directly with SYSDBA as the DB is supposed to be hanged, but starting from 11.2.0.2 onwards, hanganalyze will not produce output under a sqlplus "preliminary connection" unless you are connected to a fully logged SQLPLUS session! [Doc ID 452358.1].
...
oradebug hanganalyze ${ANALYZE_LEVEL}
--oradebug tracefile_name
--Flush any pending writes to the trace file and close it:
oradebug flush
oradebug close_trace
EOF
		 break ;;
                 *) echo "Please enter a VALID answer [1|2]";
		    echo "*********************************" ;;
                esac
