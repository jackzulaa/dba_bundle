...
# Created:	10-04-11	    #   #   # #   # 
# Modified:	28-02-19 Added Statistics status.
# 		14-05-19 Added the DDL Generation.
#
###################################################
...
SELECT object_type from dba_objects where owner=upper('${OWNER}') and object_name=upper('${OBJECT_NAME}');
EOF
)
...
${ORACLE_HOME}/bin/sqlplus -s '/ as sysdba' << EOF
set pages 0 linesize 200 heading off feedback off
col DDL_STATEMENT for a160 word_wrapped

PROMPT
PROMPT -- RETRIEVING DDL STATEMENT FOR THE TABLE AND ITS CONSTRAINTS:  [${OWNER}.${OBJECT_NAME}] ...
PROMPT -- -----------------------------------------------------------

set long 10000000
col DDL_STATEMENT for a160
...
SELECT dbms_metadata.get_ddl(upper('${OBJECT_TYPE}'),upper('${OBJECT_NAME}'),upper('${OWNER}')) DDL_STATEMENT FROM dual;

PROMPT -- DISABLE REFERENCING FK ON OTHER TABLES: [For reference only]
PROMPT -- ---------------------------------------
...
SELECT dbms_metadata.get_dependent_ddl('INDEX', upper('${OBJECT_NAME}'), upper('${OWNER}')) DDL_STATEMENT FROM dual;

PROMPT
PROMPT -- Granted Privileges:
PROMPT -- -------------------

select 'GRANT '||PRIVILEGE||' ON '||OWNER||'.'||TABLE_NAME||' TO '||GRANTEE||';' from DBA_TAB_PRIVS where owner=upper('${OWNER}') and table_name=upper('${OBJECT_NAME}');

set pages 100 heading on
...
AND    IND.OWNER = S.OWNER
AND    IND.INDEX_NAME = S.INDEX_NAME;
