...
# Created:	01-02-2013	    #   #   # #   # 
# Modified:	16-09-2014 Add Search Feature
#		13-05-2019 Enhanced the Get DDL for package type.		   
#		14-05-2019 Removed the STORAGE part from the generated DDL.
# ###########################################################################
...
DDLSCRIPT=${USR_ORA_HOME}/CREATE-${OBJECT_OWNER}.${OBJECT_NAME}.sql
...
# ##########################
${ORACLE_HOME}/bin/sqlplus -S "/ as sysdba" <<EOF
set pages 0 linesize 160 heading off feedback off
SPOOL ${DDLSCRIPT}
...
set long 10000000
col DDL_STATEMENT for a160 word_wrapped
...
select dbms_metadata.get_ddl('PACKAGE_SPEC', upper('${OBJECT_NAME}'),upper('${OBJECT_OWNER}')) from dual ;
PROMPT /
PROMPT
...
select dbms_metadata.get_ddl('PACKAGE_BODY', upper('${OBJECT_NAME}'),upper('${OBJECT_OWNER}')) from dual ;
...
EOF
;;
            "TABLE") OBJECT_TYPE=${OBJECT_TYPE1}
# Execution of SQL Statement:
# ##########################
${ORACLE_HOME}/bin/sqlplus -S "/ as sysdba" <<EOF
set pages 0 linesize 160 heading off feedback off
...
PROMPT
PROMPT -- RETRIEVING DDL STATEMENT FOR THE TABLE AND ITS CONSTRAINTS:  [${OBJECT_OWNER}.${OBJECT_NAME}] ...
PROMPT -- -----------------------------------------------------------
...
PROMPT
PROMPT -- DISABLE/DROP REFERENCING FK ON OTHER TABLES: [For reference only]
PROMPT -- --------------------------------------------

PROMPT
SELECT 'ALTER TABLE '||owner||'.'||table_name||' DISABLE CONSTRAINT '||constraint_name||';' FROM DBA_CONSTRAINTS WHERE constraint_type = 'R' 
AND r_constraint_name IN (SELECT constraint_name from DBA_CONSTRAINTS WHERE OWNER=upper('${OBJECT_OWNER}') AND table_name=upper('${OBJECT_NAME}') AND constraint_type='P');

PROMPT
select 'ALTER TABLE '||owner||'.'||table_name||' DROP CONSTRAINT '||constraint_name||';' DDL_STATEMENT from dba_constraints
where constraint_type = 'R' and r_constraint_name in (select constraint_name from dba_constraints where OWNER=upper('${OBJECT_OWNER}') AND table_name=upper('${OBJECT_NAME}') and constraint_type in ('P','U')); 
...
PROMPT
SELECT dbms_metadata.get_dependent_ddl('INDEX', upper('${OBJECT_NAME}'), upper('${OBJECT_OWNER}')) DDL_STATEMENT FROM dual;
...
PROMPT
--SELECT DBMS_METADATA.GET_DEPENDENT_DDL('OBJECT_GRANT', upper('${OBJECT_NAME}'), upper('${OBJECT_OWNER}')) FROM DUAL;
