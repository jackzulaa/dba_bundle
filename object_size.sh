...
# Created:	24-04-16	    #   #   # #   # 
# Modified:	09-05-19 Add the aggregation of total object size [indexes+lobs].
# ###############################################################################
...
LOB_COUNT_RAW=$(${ORACLE_HOME}/bin/sqlplus -s '/ as sysdba' << EOF
SELECT COUNT(*) FROM DBA_LOBS WHERE OWNER=upper('$OWNER') AND TABLE_NAME=UPPER('$OBJECT_NAME');
EOF
)
LOB_COUNT=`echo ${LOB_COUNT_RAW}| awk '{print $NF}'`
                        case ${LOB_COUNT} in
                        0) ;;
                        *)
...
(SELECT SEGMENT_NAME FROM DBA_LOBS WHERE owner=upper('${OWNER}') AND table_name=UPPER('${OBJECT_NAME}'));
exit;
EOF
)
...
col LOB_NAME format a30
comp SUM of OBJECT_SIZE_MB on report
bre on report

PROMPT ITS LOBS SIZE:
PROMPT --------------

SELECT /*+RULE*/ SEGMENT_NAME LOB_NAME, TABLESPACE_NAME, ROUND(SUM(BYTES/1024/1024)) OBJECT_SIZE_MB
FROM   SYS.DBA_SEGMENTS
WHERE  SEGMENT_NAME in (select /*+RULE*/ SEGMENT_NAME from dba_lobs where owner=upper('$OWNER') and table_name=UPPER('$OBJECT_NAME'))
...
# Aggregate Full Object Size: [Object + Indexes + Lobs]
	# If no INDEXES found set INDEXESSIZE to 0 to avoid the aggregation failure:
        if [ -z ${INDEXESSIZE} ]
        then
        export INDEXESSIZE=0
        fi

	# If no LOBS found set INDEXESSIZE to 0 to avoid the aggregation failure:
        if [ -z ${LOBSIZE} ]
        then
        export LOBSIZE=0
        fi
...
echo ""
export OBJECTSIZE=$(echo "${SINGLETABLESIZE} + ${LOBSIZE} + ${INDEXESSIZE}" | bc)
echo "[TOTAL OBJECT SIZE:  ${OBJECTSIZE} MB]"
echo ""
